...
### Add/dismiss selection playgound
...
selections from the Chips. Use this option when the full list of possible
selections is too great to reasonably display in one group of Chip. For example,
...
```ts
import { ChipDismissible } from "../Chips";
```

<Playground>
  <ChipDismissible
    prefix={<Avatar initials="MB" />}
    label="Mercedes-Benz"
    onClick={() => alert("chip")}
    onRequestRemove={() => alert("remove")}
  />{" "}
  <ChipDismissible
    prefix={<Icon name="engine" color="yellow" />}
